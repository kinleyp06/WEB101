      ...
      <div className="mb-8">
        ...
        <div className="grid grid-cols-3 gap-4">
          {Array.from({ length: 6 }).map((_, index) => (
            <div
              key={index}
              className="aspect-square bg-gray-200 rounded-md flex flex-col items-center justify-center p-4"
            >
              <p className="font-semibold">#trending{index + 1}</p>
              <p className="text-sm text-gray-500">{((index + 1) * 1.5).toFixed(1)}M views</p>
            </div>
          ))}
        </div>
      </div>
